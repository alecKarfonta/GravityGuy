	...
	public void beginContact(Contact contact) {
		...
		if (contact.getFixtureA().getUserData() instanceof Player
			...
			if (!contact.getFixtureB().isSensor()) {
				((Player)contact.getFixtureA().getUserData()).damage(10);
			}
			...
			if (!contact.getFixtureA().isSensor()) {
				((Player)contact.getFixtureB().getUserData()).damage(10);
			}
			...
			float headDifference = Math.abs(((LightBolt)contact.getFixtureB().getUserData()).getPosition().y - (((LightBolt)contact.getFixtureB().getUserData()).height / 2) - ((Lgm1)contact.getFixtureA().getUserData()).getPosition().y);
			//Gdx.app.debug(TAG, "headDistance: " + headDifference);
			if (headDifference < .1f) {
				((Lgm1) contact.getFixtureA().getUserData()).damage(100);
			} else {
